class Solution:
    ...
    def status_report(self):
        ...
        if self.first_solution is None:
            return print(f'Challenge for December {self.day} has not been started')
        self.result = self.first_solution(self.test_lines)
        if self.result != self.first_test_result:
            return self._report_failed_test(self.first_test_result)
        ...
        if self.result != self.second_test_result:
            ...
            return self._report_failed_test(self.second_test_result)
    ...
    def _report_failed_test(self, expected_result):
        print(f'Test for December {self.day} part 2: failed with actual={self.result} and expected={expected_result}')
