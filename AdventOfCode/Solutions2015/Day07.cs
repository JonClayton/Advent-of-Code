...
public partial class Day07 : Solution<long?>
{
    ...
    {
        return GeneralSolution(lines.Append("46065 -> b").ToList());
    }
    ...
    private static uint TruncateTo16Bit(uint value)
    {
        return value % uint.MaxValue;
    }

    private uint GetValue(string input)
    {
        return uint.TryParse(input, out var result) ? result : _wireSignals[input];
    }
    ...
    private bool TryFollowInstruction(Instruction instruction)
    {
        try
        {
            ...
            {
                "RSHIFT" => GetValue(instruction.Remainder[0]) >> int.Read(instruction.Remainder[1]), // OK
                "LSHIFT" => GetValue(instruction.Remainder[0]) << int.Read(instruction.Remainder[1]), // OK
