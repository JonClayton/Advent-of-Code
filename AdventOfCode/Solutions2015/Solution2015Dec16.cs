...
public class Solution2015Dec16 : Solution<long?>
{
    protected override long? FirstSolution(List<string> lines) =>
        lines.Select(line => new Sue(line)).ToList().Single(IsRightSue).Number;

    protected override long? SecondSolution(List<string> lines) =>
        lines.Select(line => new Sue(line)).ToList().Single(IsRightSue2).Number;
    ...
    private class Sue
    {
        public Sue(string input)
        {
            ...
            Number = int.Parse(input[4..firstColon]);
            var possessions = input[(firstColon + 1)..].Split(',');
            foreach (var possession in possessions)
            {
                var split = possession.Split(':');
                Possessions.Add(split[0].Trim(), int.Parse(split[1]));
            }
        }
