...
public partial class Day06 : Solution<long?>
{
    ...
    private class Instruction(GroupCollection captures)
    {
        ...
        public Vector2 Start { get; } = new(int.Read(captures[2].Value), int.Read(captures[3].Value));
        public Vector2 End { get; } = new(int.Read(captures[4].Value), int.Read(captures[5].Value));
    }
}